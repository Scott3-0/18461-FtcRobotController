 ...
 */
 ...
 //TODO:
     ...
     *
     **/
@Autonomous(name="Drive-To Andy")
public class DriveToAndyAuto extends OpMode {

    BotHardware bot = new BotHardware(this);
    private BNO055IMUHeadingSensor imu = null;

    float uniPow = 0.75f * bot.fastPow;
    float initHead = 0f;
    SensorLib.PID pid;
    ...
    public void init(){
        ...
        float initX = 3*tl -(botLength/2);//2.5f*tl-(botLength/2);
        float initY = -2f*tl;//-0.5f*tl-(botLength/2);

        //pid setup stuff
        float Kp = 0.05f;
        float Ki = 0.10f;         // ... integrator term
        float Kd = 0f;      // ... derivative term
        float KiCutoff = 7.5f;    // maximum angle error for which we update integrator
        ...
        seq.add(new AutoLib.SqPosIntDriveToStep(this, posInt, motors, uniPow, pid, new Position(DistanceUnit.INCH, initX, -0.5*tl, 0, 0), 0, tol, false));
        seq.add(new AutoLib.SqPosIntDriveToStep(this, posInt, motors, uniPow, pid, new Position(DistanceUnit.INCH, 0*tl, 0*tl, 0, 0), 0, tol, false));
